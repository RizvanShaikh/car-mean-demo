...
export class BuyerListComponent implements OnInit {
  ...
  constructor(
    ...
    titleService.setTitle(this.APPNAME + " | " + this.BUYER_CONST.MODULE_NAME);
  }
  ...
  ngAfterViewInit() {
    ...
    this._script.loadScripts('kt-buyer-list', scripts).then(function () {
      ...
      var table = $(document).find('#kt_datatable').DataTable({
        ...
        serverSide: true,
        oLanguage: {
          sProcessing: that.BUYER_CONST.MESSAGES.PROCESSING,
        },
        ...
        ajax: {
          url: ApiURIs.listProvider + "/BUYER",
          type: 'POST',
          headers: {
        ...
        columns: [
          {
            title: that.BUYER_CONST.LABEL.SR_NO, data: null
          },
          { title: that.BUYER_CONST.LABEL.USER_NAME, data: 'user_firstName' },
          { title: that.BUYER_CONST.LABEL.STATUS, data: 'user_status' },
          { title: that.BUYER_CONST.LABEL.CREATED_ON, data: 'user_createdOn' },
          { title: that.BUYER_CONST.LABEL.ACTIONS },
        ],
        ...
        initComplete: function () {
          ...
          this.api().columns().every(function () {
            ...
            var input;
            switch (column) {
              case that.BUYER_CONST.LABEL.SR_NO:
                break;
              case that.BUYER_CONST.LABEL.USER_NAME:
                input = $(`<input type="text" class="form-control form-control-sm form-filter kt-input" data-col-index="` + this.index() + `"/>`);
                break;
              case that.BUYER_CONST.LABEL.STATUS:
                input = $(`<select class="form-control form-control-sm form-filter kt-input" title="Select" data-col-index="` + this.index() + `">
                <option value="">${that.BUYER_CONST.PLACEHOLDERS.STATUS}</option>
                <option value="ACTIVE">${that.BUYER_CONST.LABEL.ACTIVE}</option>
                <option value="INACTIVE">${that.BUYER_CONST.LABEL.INACTIVE}</option></select>
                `);
                break;
              case that.BUYER_CONST.LABEL.CREATED_ON:
                input = $(`<input type="text" class="form-control form-control-sm form-filter kt-input" data-col-index="` + this.index() + `"/>`);
                break;
              case that.BUYER_CONST.LABEL.ACTIONS:
                ...
                margin-left: 10px !important; margin-top: 0px !important;">
                      <span>
                        ...
                        <span>${that.BUYER_CONST.BUTTONS.SEARCH}</span>
                      </span>
                    </button>`);
                ...
                margin-left: 10px !important; margin-top: 0px !important;">
                      <span>
                        ...
                        <span>${that.BUYER_CONST.BUTTONS.RESET}</span>
                      </span>
                    </button>`);
...
console.log(a, "c name")
  ...
  edit(e) {
    this.id = $(e).data('id');
    this.router.navigate(['admin/buyer/edit/' + this.id])
  }
  ...
  deleteSure() {
    ...
    this.store.dispatch(new buyerActions.DeleteBuyer(this.id));
    ...
    this.stateSubscription = this.store.pipe(select(fromBuyer.selectPageState)).subscribe((data) => {
      if (data && data != undefined) {
        if (data['code'] == 200) {
          ...
          const buyerTable = table.DataTable();
  ...
  updateStatus() {
    ...
    this.store.dispatch(new buyerActions.UpdateStatus(obj));
