...
import { titles, subHeader, PERMISSIONS, APPNAME } from '../../../../helper/page.title';
...
export class CmsListComponent implements OnInit {
  ...
  searchObj = { field: 'cms_title', cms_status: '', search: '' };
  ...
  //     if (data && data.length) {
  //       if (this.checkHasPermission(data, PERMISSIONS.CMS.CREATE)) {
  //         this.createPermission = true;
  //       }
  //       if (this.checkHasPermission(data, PERMISSIONS.CMS.EDIT)) {
  //         this.editPermission = true;
  //       }
  //       if (this.checkHasPermission(data, PERMISSIONS.CMS.DELETE)) {
  //         this.deletePermission = true;
  //       }
  //       if (this.checkHasPermission(data, PERMISSIONS.CMS.READ)) {
  ...
  edit(e) {
    this.id = $(e).data('id');
    this.router.navigate(['admin/cms-management/edit/' + this.id])
  }
  ...
  deleteSure() {
    ...
    this.store.dispatch(new cmsActions.DeleteCMS(this.id));
    ...
    this.stateSubscription = this.store.pipe(select(fromCMS.selectPageState)).subscribe((data) => {
      if (data && data != undefined) {
        if (data['code'] == 200) {
          ...
          const cmsTable = table.DataTable();
  ...
  updateStatus() {
    let obj = {
      data: { cms_status: this.status && this.status != undefined && this.status == 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' },
      id: this.id
    };
    Helpers.setLoading(true);

    this.store.dispatch(new cmsActions.UpdateStatus(obj));
